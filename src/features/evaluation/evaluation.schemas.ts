...
export const EvaluationScorecardSchema = z.object({
  ...
  toneScore: z.number().int().min(0).max(10).describe("Score from 0-10 on the professionalism and warmth of the custom text blocks."),
  accuracyScore: z.number().int().min(0).max(10).describe("Score from 0-10 on how accurately the chosen products match the client's requested event type, guest count, and dates."),
  finalScore: z.number().int().min(0).max(100).describe("The overall weighted score. Deduct heavily for missing requirements."),
